...
readonly vars_path="${script_path}/../../../vendor/apollo/vars"
...
main() {
  ...
  for d in ${ds}; do
    (
      ...
      source "${dv}"
      local mk="$(ls ${top}/device/google/*/apollo_${d}.mk)"
  ...
  for d in ${ds}; do
    (
      local dv="${vars_path}/${d}"
      source "${dv}"
      local dir="$(ls ${top}/device/google/*/apollo_${d}.mk | sed s#/apollo_${d}.mk##)"
      cd "${dir}"
